...
class LocalPocketBaseFallback:
    ...
    def _ensure_schema(self) -> None:
        ...
        with self._connect() as conn:
            ...
            self._seed_reservation_defaults(conn)
            conn.execute("SELECT pg_advisory_unlock(48271042)")
        self._initialized = True

    def _seed_reservation_defaults(self, conn) -> None:
        labs = [
            ("lab_fisica", "Laboratorio de Fisica", 39),
            ("lab_quimica", "Laboratorio de Quimica", 30),
            ("lab_sistemas", "Laboratorio de Sistemas", 35),
        ]
        for lab_id, name, capacity in labs:
            self._seed_record(
                conn,
                "laboratory",
                lab_id,
